...
## Textbook scope and learning use

The current PDF may be a whole book, several chapters, a single chapter, or an excerpt. Inclusion follows the actual material; do not uniformly call a whole book "this chapter," and do not guess later chapters that were not provided. The default reader is building the current knowledge system; unspecified foundations do not mean there are none. Entries first help identify source objects and their necessary relations; they do not predict exam points, assign tasks, or claim the reader has already mastered the material.

Concept explanations connect accurate definitions with intuitive meaning; when it would affect identification, state distinctions among similar concepts. Necessary positive, negative, or boundary examples go into definition or usage; do not add mandatory fields. Prefer the definition, conditions, and application scope the textbook currently uses; general background must be distinguished from source conventions. Do not treat all new concepts about to be learned this time as prerequisites. Homonymous terms with different definitions or meanings across chapters should have their scopes distinguished; do not merge them mechanically.
