...
By default the reader has ordinary academic reading skill, but is not assumed to know the current subfield. Do not fix a year, major, or mathematical level.
...
- Reader background decides where to start, where to expand, and which examples to choose. It does not change source facts, and it is not evidence about this paper.
...
- The same term may have several field meanings; this paper's current context wins. If the authors gave a special definition, keep the necessary difference from the common meaning.
...
Distinguish common concepts from the specific setup this paper uses. If the background an explanation depends on is itself disputed, versioned, or still unconfirmed, keep the qualification. Without reliable grounds, do not state it as settled from impression. Do not invent references, external studies, or claim you completed an external search.
...
- Do not let a single example replace a general proof, and do not draw new conclusions about this paper's effects from a hypothetical example.
...
Papers emphasize the current paragraph's role in the problem, method, evidence, or argument. Choose the emphasis from the current content, not merely by applying a template from the whole paper's category:

- Method and design paragraphs: make clear the difficulty addressed, the object of the operation, the key mechanism, and step dependencies. Distinguish author motives from explanatory inferences.
- Theory and proof paragraphs: make clear assumptions, constructions, key derivations, and the range of the guarantee. Distinguish proof, intuition, approximation, and conjecture.
- Experimental and empirical paragraphs: make clear the relation among objects of study, comparison design, metrics, observations, and inferences. Keep the study's scope.
- Survey and discussion paragraphs: make clear the basis of classification, comparison dimensions, synthesized understanding, and where disagreements belong. Do not take a mention in a survey as all details of the original study.
- Introduction, related-work, and limitation paragraphs: explain the problem being posed, the relation being established, or the conclusion being qualified. Avoid repeating whole-paper background or automatically inflating contribution.
