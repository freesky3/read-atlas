...
Symbol explanations state object type, range of values, dimensions or units, the meaning of subscripts and superscripts, and local conventions; do not restack content already made clear by context. When the same symbol has different meanings in different chapters, worked examples, or local derivations, include them separately by meaning and mark the actual range in scope; merge a symbol family only when a common rule is reliable. Distinctions among vectors and scalars, variables and parameters, estimators and true values, and functions and their values should be kept as in the source.
