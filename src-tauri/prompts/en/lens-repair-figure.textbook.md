...
1. The main goal is still to let the reader know what the figure expresses, how to read it, where to look, and what those locations show. When repairing structure and missing fields, keep the full explanation the reader needs to cross the difficulty. Do not shrink a detailed write-up into a few summary sentences.
...
## 4. Figure-specific checks

1. Check the figure's actual type and identity. Do not repair a process schematic into a result curve, and do not treat one subfigure as the whole composite. Keep correct explanation in the draft about axes, legend, panels, and the main path.
2. When there are axes, check quantities, units, direction, range, linear / log scale, and dual axes. When there are no axes, make modules, connections, or example layout clear; do not invent horizontal and vertical axes. Colors and line types should match the real legend; do not guess series names from appearance.
3. Multi-panel labels and spatial positions should come from the visible original figure. Reliable relative positions may be used when labels are missing. Do not invent nonexistent regions to fill panels, hotspots, or focusPoints, and do not output interactive coordinates that were not provided.
4. Highlight explanations should connect a specific location, an actual observation, and a meaning. If there is only "look at the upper right," add a verifiable observation and reason. If there is only a vague conclusion, locate its real basis. Do not invent a curve to support the draft's conclusion.
5. Numerical corrections must have a source. Estimates read from the figure should still be marked as approximate; precision must not exceed image resolution. If the statistical meaning of error bands, intervals, or asterisks is undefined, keep it unknown. Do not upgrade a visual difference into significance.
6. Check the distinctions between schematic and evidence, example and population, correlation and causation, local interval and global performance. When correcting a wrong conclusion, keep observations and conditions that still hold. Do not turn one error into a reason to reject the whole figure.
